	...
	public SqlType map(ClassHierarchy ch, JpaProperty p) {
		return p.type2.accept(new JavaTypeVisitor<SqlType>() {
			...
			public SqlType visit(RawType rawType) {
				if (rawType.getRawType().equals(JpaClassAnalyser.javaLangCharacter)) {
					return CharType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.javaLangFloat)) {
					// FIXME: driver
					return DoubleType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.javaLangDouble)) {
					return DoubleType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.javaLangInteger)) {
					return IntType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.javaLangLong)) {
					return BigIntType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.javaLangString)) {
					if (Boolean.TRUE.equals(p.lob))
						return CLOBType.create();
					else
						return VarcharType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.javaSqlDate)) {
					return DateType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.javaSqlTime)) {
					return TimeType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.javaMathBigDecimal)) {
					return DecimalType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.javaUtilDate)) {
				...
				} else if (rawType.getRawType().equals(JpaClassAnalyser.orgJodaTimeDateTime)) {
					return TimestampType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.orgJodaTimeLocalDate)) {
					return DateType.create();
				} else if (rawType.getRawType().equals(JpaClassAnalyser.orgJodaTimeLocalTime)) {
					return TimeType.create();
				} else {
					ClassModel classModel = ch.get(rawType.getRawType());
					if (classModel != null
							&& classModel.getSuperType().getRawType().equals(JpaClassAnalyser.javaLangEnum)) {
